...
var (
	...
	KEY_MINUS       = []byte{KEY_NONE, 0x00, MINUS, 0x00, 0x00, 0x00, 0x00, 0x00}

	EQUAL     = byte(0x2e) // Keyboard = and +
	KEY_PLUS  = []byte{MOD_LSHIFT, 0x00, EQUAL, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_EQUAL = []byte{KEY_NONE, 0x00, EQUAL, 0x00, 0x00, 0x00, 0x00, 0x00}

	LEFTBRACE            = byte(0x2f) // Keyboard [ and {
	KEY_LEFT_CURLY_BRACE = []byte{MOD_LSHIFT, 0x00, LEFTBRACE, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_LEFTBRACE        = []byte{KEY_NONE, 0x00, LEFTBRACE, 0x00, 0x00, 0x00, 0x00, 0x00}

	RIGHTBRACE            = byte(0x30) // Keyboard ] and }
	KEY_RIGHT_CURLY_BRACE = []byte{MOD_LSHIFT, 0x00, RIGHTBRACE, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_RIGHTBRACE        = []byte{KEY_NONE, 0x00, RIGHTBRACE, 0x00, 0x00, 0x00, 0x00, 0x00}

	BACKSLASH     = byte(0x31) // Keyboard \ and |
	KEY_PIPE      = []byte{MOD_LSHIFT, 0x00, BACKSLASH, 0x00, 0x00, 0x00, 0x00, 0x00}
	...
	KEY_COLON     = []byte{MOD_LSHIFT, 0x00, SEMICOLON, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_SEMICOLON = []byte{KEY_NONE, 0x00, SEMICOLON, 0x00, 0x00, 0x00, 0x00, 0x00}

	APOSTROPHE        = byte(0x34) // Keyboard ' and "
	KEY_DOUBLE_QUOTES = []byte{MOD_LSHIFT, 0x00, APOSTROPHE, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_APOSTROPHE    = []byte{KEY_NONE, 0x00, APOSTROPHE, 0x00, 0x00, 0x00, 0x00, 0x00}

	GRAVE         = byte(0x35) // Keyboard ` and ~
	KEY_GRAVE     = []byte{MOD_LSHIFT, 0x00, GRAVE, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_BACK_TICK = []byte{KEY_NONE, 0x00, GRAVE, 0x00, 0x00, 0x00, 0x00, 0x00}

	COMMA                  = byte(0x36) // Keyboard , and <
	KEY_LEFT_ANGLE_BRACKET = []byte{MOD_LSHIFT, 0x00, COMMA, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_COMMA              = []byte{KEY_NONE, 0x00, COMMA, 0x00, 0x00, 0x00, 0x00, 0x00}

	DOT                     = byte(0x37) // Keyboard . and >
	KEY_RIGHT_ANGLE_BRACKET = []byte{MOD_LSHIFT, 0x00, DOT, 0x00, 0x00, 0x00, 0x00, 0x00}
	KEY_PERIOD              = []byte{KEY_NONE, 0x00, DOT, 0x00, 0x00, 0x00, 0x00, 0x00}

	FORWARD_SLASH     = byte(0x38) // Keyboard / and ?
	KEY_QUESTION_MARK = []byte{MOD_LSHIFT, 0x00, FORWARD_SLASH, 0x00, 0x00, 0x00, 0x00, 0x00}
